...
# Locate the phone input field using XPath
try:
    ...
    print(f"Error finding phone input field: {e}")
    driver.quit()
    exit()

# Locate and fill the invitation code field
try:
    ...
    print(f"Error finding invitation input field: {e}")
    driver.quit()
    exit()

# Solve CAPTCHA
try:
    captcha_element = driver.find_element(By.XPATH, '//*[@id="captcha-element"]')  # Update XPath if needed
...
# Simulate human-like mouse movement before submitting
try:
